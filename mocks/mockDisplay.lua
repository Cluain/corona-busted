...
local function newDisplayObject()
    ...
    _displayObject.contentHeight = function() return _displayObject.contentBounds.yMax - _displayObject.contentBounds.yMin end
    _displayObject.contentWidth = function() return _displayObject.contentBounds.xMax - _displayObject.contentBounds.xMin end
    ...
    _displayObject.scale = function(self, xScale, yScale) _displayObject.xScale = _displayObject.xScale * xScale _displayObject.yScale = _displayObject.yScale * yScale end
    ...
    _displayObject.toBack = function() assert(_displayObject.parent, "no parent set!") local c = _displayObject.parent:remove(_displayObject) table.insert(c, 1, _displayObject) end
    _displayObject.toFront = function() assert(_displayObject.parent, "no parent set!") local c = _displayObject.parent:remove(_displayObject) table.insert(c, _displayObject) end
    _displayObject.translate = function(x, y) _displayObject.x = _displayObject.x + x _displayObject.y = _displayObject.y + y end
    ...
    _displayObject.dispatchEvent = function(self, event)
        ...
        if not _displayObject.eventListeners[event.name] or #_displayObject.eventListeners[event.name] == 0 then return end

        for i = 1, #_displayObject.eventListeners[event.name] do
            _displayObject.eventListeners[event.name][i](event)
        end
    end
    ...
    local metatable = {
        ...
        __newindex = function(t, key, value)
            if key == "x" then
                if _displayObject.referencePoint == display.TopLeftReferencePoint or _displayObject.referencePoint == display.CenterLeftReferencePoint or _displayObject.referencePoint == display.BottomLeftReferencePoint then
                    _displayObject.contentBounds.xMin = value
                    _displayObject.contentBounds.xMax = value + _displayObject.width
                elseif _displayObject.referencePoint == display.TopRightReferencePoint or _displayObject.referencePoint == display.CenterRightReferencePoint or _displayObject.referencePoint == display.BottomRightReferencePoint then
            ...
            elseif key == "y" then
                if _displayObject.referencePoint == display.TopLeftReferencePoint or _displayObject.referencePoint == display.TopCenterReferencePoint or _displayObject.referencePoint == display.TopRightReferencePoint then
                    _displayObject.contentBounds.yMin = value
                    _displayObject.contentBounds.yMax = value + _displayObject.height
                elseif _displayObject.referencePoint == display.CenterLeftReferencePoint or _displayObject.referencePoint == display.CenterReferencePoint or _displayObject.referencePoint == display.CenterRightReferencePoint then
...
local currentStageObject = {
    focused = nil,
}
currentStageObject.setFocus = function(self, what)
    currentStage.focused = what
end
...
display = {
    newRect = function(x, y, width, height)
        ...
        newRect.setFillColor = function(self, r, g, b)
            newRect.fillColor = { r, g, b }
        end
        newRect.setStrokeColor = function(self, r, g, b)
            newRect.strokeColor = { r, g, b }
        end
        return rect
    end,
    newGroup = function()
        ...
        group.contentWidth = function()
            ...
            if (#group.children == 1) then return group.children[1].contentBounds.xMax - group.children[1].contentBounds.xMin end
            local xMin, xMax
            xMin = #group.children[1].contentBounds.xMin
            xMin = #group.children[1].contentBounds.xMax
        ...
        group.contentHeight = function()
            if (#group.children == 0) then return 0 end
            if (#group.children == 1) then return group.children[1].contentBounds.yMax - group.children[1].contentBounds.yMin end
            local yMin, yMax
            yMin = #group.children[1].contentBounds.yMin
            yMin = #group.children[1].contentBounds.yMax
            for i = 2, #group.children do
    ...
    newImage = function(sheet, frame)
        local image = newDisplayObject()
        local info = sheet.spriteInfo.frames[frame]
    ...
    currentStage = currentStageObject,
